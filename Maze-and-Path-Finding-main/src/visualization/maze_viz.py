...
def plot_maze(
    ...
    *,
    ...
    ----------
    maze_or_env:
        Either a raw maze array from ``generate_maze`` or an existing
        ``MazeEnv`` instance. In the maze array, ``1`` means wall and ``0``
        means free cell.
    path:
        Optional sequence of ``(row, col)`` cells to draw in orange. If omitted
        and ``show_solution`` is true, the BFS shortest path is drawn.
    show_solution:
        Compute and draw the BFS shortest path when ``path`` is not provided.
    start_pos, goal_pos:
        Optional overrides. Defaults to ``MazeEnv.start_pos``/``goal_pos`` when
        available, otherwise top-left and bottom-right.
    title:
        Optional plot title.
    ax:
...
if __name__ == "__main__":
    from src.env.maze_generator import generate_maze

    demo_maze = generate_maze(size=15, seed=42, difficulty="medium")
    show_maze(demo_maze, title="Generated maze")
