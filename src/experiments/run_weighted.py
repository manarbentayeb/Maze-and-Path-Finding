"""Run Weighted-Cost Maze training.

Example:
    python -m src.experiments.run_weighted --algo qlearning --level small --episodes 2000 --seed 42
"""
...
from src.visualization.heatmaps import plot_q_heatmap_weighted
from src.visualization.policy_maps import plot_policy_arrows_weighted
from src.visualization.maze_viz import plot_weighted_maze


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train agent on Weighted-Cost maze.")
    ...
    # Env params
    parser.add_argument("--mud-density", type=float, default=0.2)
    parser.add_argument("--mud-penalty", type=float, default=0.05)
...
def main() -> None:
    ...
    run_id = f"weighted_{args.algo}_{level_part}_{args.difficulty}_seed{args.seed}"
...
def _save_plots(run_id: str, env: WeightedMazeEnv, agent, result, algo: str) -> None:
    if algo == "qlearning":
    ...
    figures = {
        ...
        "q_heatmap": plot_q_heatmap_weighted(q_table, env.maze, title=f"{algo.upper()} Q-value heatmap"),
        "policy_map": plot_policy_arrows_weighted(q_table, env.maze, title=f"{algo.upper()} greedy policy"),
    }
    
    # Add maze layout plot
    fig, _ = plot_weighted_maze(env, title=f"Weighted Maze Layout ({run_id})")
